...
.fbgrid button{min-height:96px;border:1.5px solid var(--line);border-radius:16px;background:var(--card);color:var(--ink);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;padding:12px 8px;text-align:center;font-size:17px;font-weight:800;line-height:1.2}
.fbgrid button .fe{font-size:32px;line-height:1}
...
function fbRun(id){
  const back=()=>fbMenu();
  switch(id){
    case"pay":closeSheet();entryForm(null,{kind:"payment"});break;
    case"exp":closeSheet();entryForm(null,{kind:"material"});break;
...
// Κλήση πελάτη με αναζήτηση
function fbCall(back){
  const all=S.clients.filter(c=>alive(c)&&(c.mobile||c.phone));
  const open=new Set(S.tasks.filter(isOpen).map(x=>x.clientId));
  all.sort((a,b)=>(open.has(b.id)-open.has(a.id))||a.name.localeCompare(b.name,LOC()));
  openSheet({title:`📞 ${T("Κλήση πελάτη")}`,cancelLabel:T("Πίσω"),onCancel:back||null,
    body:`<input id="fb_q" placeholder="${T("Όνομα ή τηλέφωνο")}" autocomplete="off" style="font-size:17px"><div id="fb_cl" class="fbpick" style="margin-top:10px"></div>`});
  const paint=()=>{const q=norm(val("fb_q"));const l=all.filter(c=>!q||norm(c.name).includes(q)||String(c.mobile||c.phone).replace(/\s/g,"").includes(q.replace(/\s/g,""))).slice(0,40);
    $("#fb_cl").innerHTML=l.length?panel(l.map(c=>`<a class="row" data-act="noop" href="tel:${esc(String(c.mobile||c.phone).replace(/\s+/g,""))}" style="text-decoration:none;color:inherit"><div class="grow"><div class="title">${esc(c.name)}</div><div class="meta"><span>${esc(c.mobile||c.phone)}</span>${open.has(c.id)?`<span>🛠️ ${T("ανοιχτή δουλειά")}</span>`:""}</div></div><span style="font-size:22px">📞</span></a>`).join(""))
      :`<div class="empty">${T("Κανένας πελάτης με τηλέφωνο.")}</div>`};
...
function fbSetup(back){
  ...
  const paint=()=>{
    $("#fbS").innerHTML=`<label class="toggle" style="font-size:16.5px"><input type="checkbox" id="fb_on" ${c.on!==false?"checked":""}>${T("Να φαίνεται το κίτρινο κουμπί κάτω δεξιά")}</label>
      <p class="note" style="font-size:14.5px;margin:6px 2px 10px">${T("Πάτα για να το βάλεις ή να το βγάλεις από το μενού. Με ▲▼ αλλάζεις σειρά.")}</p>`+
  ...
  $("#fbS").onclick=e=>{const b=e.target.closest("button");if(!b)return;
    if(b.dataset.fbk){const id=b.dataset.fbk;c.off=c.off.includes(id)?c.off.filter(x=>x!==id):c.off.concat(id);write();paint();return}
